...
1. **`/grill-with-docs`** — sharpen the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No working directory? Use `/grilling` directly — see Standalone. Both run the same `/grilling` primitive; `grill-with-docs` is the one that leaves a paper trail, which makes it the better of the two whenever a repo is there to leave it in.)
...
- **Something's broken** → **`/debug-root-cause`**. Root cause before fix, minimal fix before refactor. For the hard ones — the bug that resists a first glance, the intermittent flake, the regression that crept in between two known-good states — it hypothesises, rules hypotheses out with evidence, then lands the minimal fix. Hands off to **`/improve-codebase-architecture`** when the real finding is that there's no good seam to lock the bug down.
...
- **`/grilling`** — the interview primitive itself: rounds, the frontier, facts are the agent's job and decisions are yours. Reach for it directly when you are **not working in a working directory** — sharpening a plan, a design, a piece of writing, anything with no repo under it — it saves nothing locally, no `CONTEXT.md`. `grill-with-docs` and `/triage`, `/wayfinder`, `/improve-codebase-architecture` all run it internally with a paper trail on top; if you are in a working directory, use `/grill-with-docs` instead — same interview, leaves a paper trail, strictly the better one.
